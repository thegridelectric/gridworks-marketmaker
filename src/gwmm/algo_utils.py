...
import gwmm.errors as errors
import gwmm.property_format as property_format
from gwmm import config
...
def get_balances(client: AlgodClient, addr: str) -> Dict[int, int]:
    ...
    if not property_format.is_algo_address_string_format(addr):
        raise ValueError(f"addr does not have AlgoAddressStringFormat: {addr}")
...
def algos(addr: str) -> Optional[int]:
    ...
    """
    if not property_format.is_algo_address_string_format(addr):
        raise ValueError(f"not is_algo_address_string_format! {addr} ")
